...
export default {
  ...
  methods: {
    ...
    highLoad() {
      ...
      param.search = this.dynamicValidateForm.name+','
      for(let i = 0 ; i < this.dynamicValidateForm.domains.length ; i++){
        param.search += this.dynamicValidateForm.domains[i].kpOpVal+','
        param.search += this.dynamicValidateForm.domains[i].titOpVal+','
        param.search += this.dynamicValidateForm.domains[i].value+','
      }
      param.search = param.search.substr(0,param.search.length-1)
      console.log(this.dynamicValidateForm.domains)
      console.log(param.search)
      request.get("/book/highBook", {
        params: param
      }).then(res => {
        this.loading = false
        this.tableData = res.data.records
